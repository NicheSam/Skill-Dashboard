...
- If `npm run launch` times out, inspect `.logs/launcher-state.json` first. Inspect `.logs/server.err.log` and `.logs/client.err.log` only when they exist and have fresh timestamps.
